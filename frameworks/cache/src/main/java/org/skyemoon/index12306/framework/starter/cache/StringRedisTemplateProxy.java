...
import org.skyemoon.index12306.framework.starter.bases.Singleton;
import org.skyemoon.index12306.framework.starter.cache.config.RedisDistributedProperties;
import org.skyemoon.index12306.framework.starter.cache.core.CacheGetFilter;
import org.skyemoon.index12306.framework.starter.cache.core.CacheGetIfAbsent;
...
import org.skyemoon.index12306.framework.starter.cache.toolkit.FastJson2Util;
...
public class StringRedisTemplateProxy implements DistributedCache {
    ...
    public <T> T get(String key, Class<T> clazz) {
        ...
        if (String.class.isAssignableFrom(clazz)) { // 判断String类是否是clazz的超类或超接口，或者二者是否一样
            return (T) value;
        }
    ...
    public <T> T safeGet(String key, Class<T> clazz, CacheLoader<T> cacheLoader, long timeout, TimeUnit timeUnit, RBloomFilter<String> bloomFilter, CacheGetFilter<String> cacheCheckFilter, CacheGetIfAbsent<String> cacheGetIfAbsent) {
        T result = get(key, clazz);
        // 缓存结果不为空或空字符串直接返回；通过函数判断是否返回空，为了适配布隆过滤器无法删除的场景；两者都不成立，判断布隆过滤器是否存在，不存在返回空
        if (!CacheUtil.isNullOrBlank(result)
