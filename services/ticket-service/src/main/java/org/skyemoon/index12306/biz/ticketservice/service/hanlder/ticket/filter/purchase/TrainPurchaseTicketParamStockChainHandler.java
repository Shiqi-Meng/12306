...
public class TrainPurchaseTicketParamStockChainHandler implements TrainPurchaseTicketChainFilter<PurchaseTicketReqDTO> {
    ...
    public void handler(PurchaseTicketReqDTO requestParam) {
        // 车站站点手机否还有余票 如果用户提交多个乘车人非同一个座位类型，拆分验证
        ...
        seatTypeMap.forEach((seatType, passengerSeatDetails) -> {
            Object stockObj = stringRedisTemplate.opsForHash()
            ...
            int stock = Optional.ofNullable(stockObj)
                    ...
                    .orElseGet(() -> {
                Map<String, String> seatMarginMap = seatMarginCacheLoader.load(String.valueOf(
                        requestParam.getTrainId()),
                        String.valueOf(seatType),
                        requestParam.getDeparture(),
                        requestParam.getArrival()
                );
                return Optional.ofNullable(seatMarginMap
                        .get(String.valueOf(seatType)))
