...
public class TicketAvailabilityCacheUpdateHandler implements AbstractExecuteStrategy<CanalBinlogEvent, Void> {
    ...
    public void execute(CanalBinlogEvent message) {
        ...
        // 更新列车站点余票缓存
        // 追求性能需要将逻辑写入lua脚本，使用管道命令执行
        // binlog 可能一次触发多个数据变更，将这些操作整理至一个 Map，以进行统一操作
        ...
        cacheChangeKeyMap.forEach((cacheKey, cacheVal) -> cacheVal.forEach((seatType, num) -> stringRedisTemplate.opsForHash().increment(cacheKey, String.valueOf(seatType), num)));
    }
