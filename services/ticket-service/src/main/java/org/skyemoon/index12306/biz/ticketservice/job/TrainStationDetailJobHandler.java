package org.skyemoon.index12306.biz.ticketservice.job;
...
import org.skyemoon.index12306.biz.ticketservice.dao.entity.TrainDO;
import org.skyemoon.index12306.biz.ticketservice.dao.entity.TrainStationRelationDO;
import org.skyemoon.index12306.biz.ticketservice.dao.mapper.TrainStationRelationMapper;
import org.skyemoon.index12306.biz.ticketservice.job.base.AbstractTrainStationJobHandlerTemplate;
...
import static org.skyemoon.index12306.biz.ticketservice.common.constant.RedisKeyConstant.TRAIN_STATION_DETAIL;
    ...
    protected void actualExecute(List<TrainDO> trainDOPageRecords) {
        for (TrainDO each : trainDOPageRecords) {
            ...
            for (TrainStationRelationDO item : trainStationRelationDOList) {
                ...
                String buildCacheKay = TRAIN_STATION_DETAIL + StrUtil.join("_", each.getId(), item.getDeparture(), item.getArrival());
                stringRedisTemplate.opsForHash().putAll(buildCacheKay, actualCacheHashValue);
                stringRedisTemplate.expire(buildCacheKay, ADVANCE_TICKET_DAY, TimeUnit.DAYS);
            }
        }
    }
}
